...
import {Animated, Dimensions, Easing, PanResponder, StyleSheet, TouchableOpacity, View, ScrollView,Platform, Text} from 'react-native'
...
export default class AutoDragSortableView extends Component{
    ...
    constructor(props) {
        ...
        for (var index in props.dataSource){
            const item = props.dataSource[index];
            const newData = {}
            const left = item.isHeader ? 0 : (currentColumns % rowNum) * itemWidth;
            let top = parseInt((currentColumns / rowNum)) * itemHeight

            newData.columnPosition = currentColumns;

            if (item.isHeader ){
                if ( currentColumns % rowNum !== 0) { 
                    currentColumns += (rowNum - (currentColumns % rowNum));
                    top += itemHeight;
                }
                currentColumns += rowNum;
            }
            else{
                currentColumns += 1;
            }
        ...
        this.state = {
            ...
            height: Math.ceil(currentColumns / rowNum) * itemHeight,
            itemWidth,
            itemHeight,
    ...
    startTouch(touchIndex) {
        ...
        if ((fixedItems.length > 0 && fixedItems.includes(touchIndex))
             || this.state.dataSource[touchIndex].data.isFixed || this.state.dataSource[touchIndex].data.isHeader){
            return;
        }
    ...
    moveTouch (nativeEvent,gestureState) {
        ...
        if (this.touchCurItem) {
            ...
            const maxHeight = itemHeight*Math.ceil(this.state.totalColumns/rowNum) - itemHeight;
            ...
            if (this.touchCurItem && this.touchCurItem.ref && this.touchCurItem.ref.setNativeProps){
            ...
            if (this.touchCurItem.moveToIndex != moveToIndex ) {
                ...
                if ((fixedItems.length > 0 && fixedItems.includes(moveToIndex))
                    || this.state.dataSource[moveToIndex].data.isFixed || this.state.dataSource[moveToIndex].data.isHeader){
                   return;
               }
    ...
    _renderItemView = () => {
        ...
        return this.state.dataSource.map((item,index)=>{
            ...
            // Add special handling for header items
            ...
            return (
                <Animated.View
                    ...
                    style={[styles.item,{
                        ...
                        width: item.data.isHeader ? "100%" : null
                    }]}>
                    <TouchableOpacity
                        activeOpacity = {1}
